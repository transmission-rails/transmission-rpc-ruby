describe Transmission::Arguments::TorrentGet do
  ...
  describe '#new' do
    ...
    describe 'when fields are declared' do
      ...
      before :each do
        @model = Transmission::Arguments::TorrentGet.new(['id'])
      end
    ...
    describe 'when no fields are declared' do
      before :each do
        @model = Transmission::Arguments::TorrentGet.new
      end

      it 'should only use the declared fields' do
        expect(@model.arguments.size).to eq(68)
      end
    end
    ...
    describe 'when wrong fields are declared' do
      it 'should raise an exception' do
        expect {
          @model = Transmission::Arguments::TorrentGet.new(['id', 'i-dont-exist'])
  ...
  describe '#to_arguments' do
    ...
    it 'should return all the arguments' do
      expect(@model.to_arguments.size).to eq(68)
    end
